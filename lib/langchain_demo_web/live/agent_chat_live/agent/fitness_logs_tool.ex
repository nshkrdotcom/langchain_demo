defmodule LangChainDemoWeb.AgentChatLive.Agent.FitnessLogsTool do
  ...
  def execute_get_fitness_logs(%{} = args, %{live_view_pid: pid, current_user: user} = _context) do
    ...
    send(pid, {:function_run, "Retrieving fitness history."})
      ...
      case result do
      {:error, reason} ->
        IO.inspect("Error in FitnessLogs.list_fitness_logs", limit: :infinity)
      ...
      logs ->
        case Jason.encode(logs) do
          ...
          {:error, reason} ->
            ...
            IO.inspect("Error encoding logs to JSON", limit: :infinity)
            IO.inspect(logs, label: "Logs", limit: :infinity)
            IO.inspect(reason, label: "Encoding Reason", limit: :infinity)
            {:error, "An error occurred while encoding the fitness logs."}
        end
      end
    catch
      e, stacktrace ->
      IO.inspect("Exception in execute_get_fitness_logs", limit: :infinity)
